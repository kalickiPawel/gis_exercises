...
def get_data(url, wfs, v, layer):
    ...
    if not wfs:
        ...
        print(f"Dataset name: {wms[layer].title}")
        print(f"CRS: {wms[layer].crsOptions}")
        return wms, wms[layer], wms[layer].title.replace(" ", "_").lower()
    else:
        ...
        print(f"Dataset name: {wfs[layer].title}")
        print(f"CRS: {wfs[layer].crsOptions}")
        return wfs, wfs[layer], wfs[layer].title.replace(" ", "_").lower()
